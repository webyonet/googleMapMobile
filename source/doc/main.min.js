...
jQuery.fn.extend({
    highlight: function (a, c, d) {
        a = new RegExp("(<[^>]*>)|(\\b" + a.replace(/([-.*+?^${}()|[\]\/\\])/g, "\\$1") + ")", c ? "ig" : "g");
        return this.html(this.html().replace(a, function (b, f, e) {
            return b.charAt(0) == "<" ? b : '<strong class="' + d + '">' + e.toLowerCase() + "</strong>"
        }))
    }
});
...
jqapi = function () {
    ...
    function q(a) {
        a = a.attr("href");
        return a.substr(5, a.length - 16)
    }
    function r(a) {
        f.html(j).load(a.attr("href"), function () {
            document.title = k + a.children("span:first").text();
//            pageTracker._trackPageview(q(a));
            s()
        })
    }
    function s() {
        var a = $("p.desc", f);
        $(".arguement:odd", f).addClass("arguement-odd");
        a.text().length <= 13 && a.remove();
        $("img", f).attr("src", function () {
            return $(this).attr("src").substr(1)
        });
        $(".signatures", f).each(function () {
            var a = 0,
                b = $(this).find(".arguement");
            b.children("strong").each(function () {
                var b = $(this).width();
                b > a && (a = b)
            });
            b.css("padding-left", a + 50)
        });
        t()
    }
    function t() {
        $("code.demo-code", f).each(function () {
            var a = $(this),
                c = a.parent().parent().find(".code-demo"),
                a = a.html().replace(/<\/?a.*?>/ig, "").replace(/<\/?strong.*?>/ig, "").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&").replace("/scripts/jquery-1.4.js", "js/jquery.min.js").replace(/<script>([^<])/g, "<script>window.onload = (function(){\ntry{$1").replace(/([^>])<\/sc/g, "$1\n}catch(e){}});</sc").replace("</head>", "<style>html,body{border:0; margin:0; padding:10px; background: #FFE0BB;}</style></head>"),
                b = jQuery("<iframe>", {
                    src: "blank.html",
                    width: "100%",
                    css: {
                        height: c.attr("rel") || 125,
                        border: "none"
                    }
                }).get(0);
            c.html(b);
            c = b.contentDocument || b.contentWindow && b.contentWindow.document || b.document || null;
            c != null && (c.open(), c.write(a), c.close())
        })
    }
    function u() {
        d.doTimeout("text-type", 300, function () {
            var a = d.val();
            if (a.length) {
                g.html("").show();
                e.hide();
                var c = 100,
                    f = $;
                $(".searchable, .desc", e).each(function () {
                    var b = $(this),
                        d = b.text(),
                        e = d.toLowerCase().indexOf(a.toLowerCase());
                    e != -1 && g.text().indexOf(d) == -1 && (b = jQuery("<li>", {
                        "class": "sub",
                        html: b.parent().parent().html()
                    }).appendTo(g), e < c && (c = e, f = b))
                });
                g.prepend(f).highlight(a, !0, "highlight").children("li:first").addClass(b);
				//$('#content').highlight(a, !0, "highlight").children("li:first").addClass(b);
                $(".sub:odd", g).addClass("odd")
            } else g.hide(), e.show()
        })
    }
    ...
    return {
        initialize: function () {
            ...
            o.resize(function () {
                ...
                a.pageX < e.width() && d.focus()
            }).keydown(function (a) {
                a.keyCode == 27 && (d.val("").focus(), g.hide(), e.show())
            }).trigger("resize");
            d.keyup(function (a) {
                if ($.inArray(a.keyCode, m) != -1) {
                    if (a = a.keyCode, n) {
                        var c = $("." + b + ":visible");
                        c.length ? (a == 38 && c.prev().length && c.removeClass(b).prev().addClass(b), a == 40 && c.next().length && c.removeClass(b).next().addClass(b), a == 13 && $.bbq.pushState({
                            p: q(c.children("a"))
                        })) : (c = $(".cat-selected", e), c.length ? (a == 38 && c.removeClass("cat-selected").prev().addClass("cat-selected"), a == 40 && c.removeClass("cat-selected").next().addClass("cat-selected"), a == 13 && c.removeClass("cat-selected").children("span").trigger("click")) : (c = $(".sub:visible", e), c.length ? (a == 38 && c.filter(":last").addClass(b), a == 40 && c.filter(":first").addClass(b)) : (a == 38 && l.last().addClass("cat-selected"), a == 40 && l.first().addClass("cat-selected"))))
                    }
                } else u()
            }).focus(function () {
                n = !0
            ...
            $(".category > span", e).toggle(function () {
                p();
                d.focus();
...
$(document).ready(function () {
    ...
    $("#content").scroll(function () {
        $(this).scrollTop() > 30 ? h.fadeOut("fast") : h.fadeIn("fast")
    })
});